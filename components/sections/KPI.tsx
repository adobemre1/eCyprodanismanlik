...
export const KPI: React.FC = () => {
  return (
    <section id="kpi" className="py-24 lg:py-32 bg-primary text-white relative overflow-hidden" aria-labelledby="kpi-heading">
      {/* Background graphic */}
          ...
          {STATS.map((stat, idx) => (
            <FadeIn key={stat.id} delay={idx * 100}>
              <div className="flex flex-col h-full group relative pl-6 border-l border-slate-700/50 hover:border-secondary transition-colors duration-500">
                <div className="text-[3.75rem] lg:text-[4.5rem] leading-none font-serif font-bold text-white mb-6 group-hover:text-secondary transition-colors duration-300">
                  {stat.value}
                </div>
